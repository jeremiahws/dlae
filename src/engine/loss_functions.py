...
"""src/engine/loss_functions.py

Contains additional loss functions not natively part of Keras.
"""
...
class TverskyLoss:
    ...
    def compute_loss_2D(self, y_true, y_pred):
        ones = keras.backend.tensorflow_backend.ones(keras.backend.tensorflow_backend.shape(y_true))
        p0 = y_pred
        p1 = ones - y_pred
        g0 = y_true
        g1 = ones - y_true
    ...
    def compute_loss_3D(self, y_true, y_pred):
        ones = keras.backend.tensorflow_backend.ones(keras.backend.tensorflow_backend.shape(y_true))
        p0 = y_pred
        p1 = ones - y_pred
        g0 = y_true
...
# All copyrights to this function are reserved by the original author.
class SSDLoss:
    '''
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        self.neg_pos_ratio = tf.constant(int(self.neg_pos_ratio), dtype=tf.int32)
        self.n_neg_min = tf.constant(int(self.n_neg_min), dtype=tf.int32)
